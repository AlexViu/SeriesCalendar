package com.aledom.seriescalendar;

public class Constants {

    public static String BASE_URL = "http://192.168.1.57/SeriesCalendar/";
    public static String LOGIN_URL =  BASE_URL + "login.php";
    public static String SIGN_UP_URL = BASE_URL + "signup.php";
    public static String GET_SESION_URL = BASE_URL + "getsesion.php";
    public static String ADD_SERIE_URL = BASE_URL + "serieregister.php";
    public static String GET_SERIE_URL = BASE_URL + "series.php";
    public static String GET_SEASON_URL = BASE_URL + "getSeason.php";
    public static String ADD_SEASON_URL = BASE_URL + "addSeason.php";
    public static String GET_CHAPTER_URL = BASE_URL + "getchapter.php";
    public static String ADD_CHAPTER_URL = BASE_URL + "addchapter.php";
    public static String ADD_FAVORITE_URL = BASE_URL + "addfavorite.php";
    public static String GET_FAVORITE_URL = BASE_URL + "getfavorite.php";
    public static int SUCCESS_PETITION = 1;
    public static int ERROR_PETITION = 0;

}
